    ...
    <div style="padding:20px;border-top:2px solid var(--border);background:linear-gradient(135deg,var(--surface),var(--surface2));display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
        ...
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
            ...
            <div style="display:flex;gap:4px;flex-wrap:wrap;">
                @foreach($metrics->getUrlRange(1, $metrics->lastPage()) as $page => $url)
                    @if($page == $metrics->currentPage())
                    ...
                    @else
                        <a href="{{ $url }}" style="padding:8px 12px;border-radius:6px;background:var(--surface2);color:var(--text);text-decoration:none;font-size:12px;font-weight:600;min-width:36px;text-align:center;display:inline-block;transition:all 0.2s;"
